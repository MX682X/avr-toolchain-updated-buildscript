...
if [[ ! -f ${DOWNLOAD_DIR}/${ATMEL_ATTINY_PACK_FILENAME}.zip  ]] ; then 	# check if we already downloaded the tar
    	wget ${ATMEL_ATTINY_PACK_URL} -P ${DOWNLOAD_DIR}
    	
    	mkdir -p typack
	rm -rf ./typack/*

	mv ${DOWNLOAD_DIR}/${ATMEL_ATTINY_PACK_FILENAME}.atpack ${DOWNLOAD_DIR}/${ATMEL_ATTINY_PACK_FILENAME}.zip
	unzip -q ${DOWNLOAD_DIR}/${ATMEL_ATTINY_PACK_FILENAME}.zip -d ./typack
fi
cd ./typack
...
EXTRA_INCLUDES=`diff -q ../objdir/avr/include/avr ../typack/include/avr | grep "Only in" | grep atpack | cut -f4 -d" "`
