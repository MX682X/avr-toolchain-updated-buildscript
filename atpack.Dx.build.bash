...
if [[ ! -f ${DOWNLOAD_DIR}/${ATMEL_DX_PACK_FILENAME}.zip  ]] ; then 	# check if we already downloaded the tar
    	wget ${ATMEL_DX_PACK_URL} -P ${DOWNLOAD_DIR}
    	
    	mkdir -p dxpack
	rm -rf ./dxpack/*

	mv ${DOWNLOAD_DIR}/${ATMEL_DX_PACK_FILENAME}.atpack ${DOWNLOAD_DIR}/${ATMEL_DX_PACK_FILENAME}.zip
	unzip -q ${DOWNLOAD_DIR}/${ATMEL_DX_PACK_FILENAME}.zip -d ./dxpack
fi
...
EXTRA_INCLUDES=`diff -q ../objdir/avr/include/avr ../dxpack/include/avr | grep "Only in" | grep atpack | cut -f4 -d" "`
